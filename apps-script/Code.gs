 ...
 *       property (File → Project Settings → Script Properties).
 *
...
var REQUEST_COLS = ['id', 'employeeId', 'employeeName', 'approverId', 'type', 'otherLabel',
  'startDate', 'endDate', 'days', 'reason', 'status', 'submittedAt', 'decidedBy', 'decidedAt'];
...
function doGet(e) {
  try {
    ...
    if (action === 'getData') {
      return json_({ ok: true, users: readUsersSafe_(), requests: readRequests_() });
    }
...
function doPost(e) {
  try {
    ...
    switch (body.action) {
      ...
      case 'decideRequest': return json_(decideRequest_(body.id, body.status, body.deciderName));
...
function submitRequest_(req) {
  var sheet = sheet_(REQUESTS_SHEET);
  sheet.appendRow(REQUEST_COLS.map(function (c) { return req[c] != null ? req[c] : ''; }));
  return { ok: true };
}

function decideRequest_(id, status, deciderName) {
  return updateRowById_(REQUESTS_SHEET, id, {
    status: status, decidedBy: deciderName || '', decidedAt: new Date().toISOString(),
  });
}

function cancelRequest_(id) {
  return deleteRowById_(REQUESTS_SHEET, id);
}
...
function addUser_(user) {
  ...
  sheet.appendRow(USER_COLS.map(function (c) { return clean[c]; }));
...
function sheet_(name) {
  ...
  if (!sh) {
    ...
    sh.appendRow(name === USERS_SHEET ? USER_COLS : REQUEST_COLS);
  }
  // Ensure header row exists.
  if (sh.getLastRow() === 0) sh.appendRow(name === USERS_SHEET ? USER_COLS : REQUEST_COLS);
  return sh;
}
...
function readUsers_() {
  return readObjects_(USERS_SHEET, USER_COLS).map(function (u) {
    u.id = Number(u.id);
...
function readRequests_() {
  return readObjects_(REQUESTS_SHEET, REQUEST_COLS).map(function (r) {
    r.id = Number(r.id);
...
function sanitize_(u) {
  return { id: Number(u.id), name: u.name, username: u.username, role: u.role,
    approverId: u.approverId === '' || u.approverId == null ? null : Number(u.approverId),
    startDate: u.startDate };
}

// Update the row whose `id` column matches; patch is a {col: value} map.
...
/**
 * Run ONCE from the Apps Script editor (select setup → Run) to create the tabs
 * and seed the initial accounts so you can log in. Safe to re-run: it only adds
 * the seed users if the Users sheet is empty.
 */
function setup() {
  sheet_(USERS_SHEET);
  ...
  if (readUsers_().length === 0) {
    ...
    seed.forEach(function (u) { sh.appendRow(USER_COLS.map(function (c) { return u[c]; })); });
  }
}
